...
## Tests
- There are test skeletons under `src/test/java/...`. Use `mvn test` to run tests. Consider adding integration tests for auth and appointment flows to strengthen interview demos.

## Resume / Interview-friendly Project Description
- Short bullet for resume:
  - "Hospital Management System — Spring Boot (Java 21) backend implementing JWT + OAuth2 authentication, role-based access control, and appointment management. Modeled domain entities with JPA, used ModelMapper for DTO mapping, and secured endpoints via custom JWT filter and Spring Security. Integrated PostgreSQL and demonstrated layered architecture (Controller → Service → Repository)."

- Two-line elevator pitch you can use in interviews:
  - "I built a Hospital Management System backend using Spring Boot and PostgreSQL with JWT-based authentication and OAuth2 sign-in. The service demonstrates layered architecture, role-based security, and real-world patterns like DTO mapping, exception handling, and token-based session management."

## Suggested talking points for interviews
- Explain how JWT is generated and validated (`Authutil`, `JwtAuthFilter`).
- Point out `User` implementing `UserDetails` and how roles map to authorities via `RoleToPermissionMap`.
- Discuss how OAuth2 signups are handled (provider ID mapping & account linking) in `Authservice`.
- Mention DTO-to-entity mapping with `ModelMapper`, transactional service methods, and the pagination example in `Patientservice`.
- Highlight the DB index on `User` (provider fields) and where to add more indexes for production workloads.

## Next improvements (optional enhancements to mention)
- Add integration tests for authentication and appointment flows.
- Add front-end (React/Vue) and role-based UI views.
- Use refresh tokens and token rotation for improved security.
- Add OpenAPI (Swagger) documentation and API tests.

## Where to look next in the codebase
- Controllers: [src/main/java/com/example/hms/controller](src/main/java/com/example/hms/controller)
- Security & auth: [src/main/java/com/example/hms/security](src/main/java/com/example/hms/security)
- Entities: [src/main/java/com/example/hms/entity](src/main/java/com/example/hms/entity)
- Repositories: [src/main/java/com/example/hms/repo](src/main/java/com/example/hms/repo)

---
If you want, I can:
- Translate this README into professional Hindi, or
- Expand the Endpoints section into a full API reference (paths, request/response DTO examples), or
- Add a short `docs/ARCHITECTURE.md` with diagrams.
